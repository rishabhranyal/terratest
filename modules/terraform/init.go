...
func CustomInitE(t testing.TestingT, options *Options) (string, error) {
	args := []string{"init", fmt.Sprintf("-plugin-dir=%s", "/home/rishabh/go/bin"), fmt.Sprintf("-plugin-dir=%s", "/usr/local/bin"), fmt.Sprintf("-plugin-dir=%s", ".terraform/plugins/linux_amd64"), fmt.Sprintf("-plugin-dir=%s", "/home/rishabh/.terraform.d/plugins"), fmt.Sprintf("-upgrade=%t", options.Upgrade)}
	args = append(args, FormatTerraformBackendConfigAsArgs(options.BackendConfig)...)
	return RunTerraformCommandE(t, options, args...)
}
