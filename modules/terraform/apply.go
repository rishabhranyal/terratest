...
//CustomInitAndApply runs terraform init (with custom azurerm plugin) and apply with the given options and return stdout/stderr from the apply command. Note that this
// method does NOT call destroy and assumes the caller is responsible for cleaning up any resources created by running
// apply.
func CustomInitAndApply(t testing.TestingT, options *Options) string {
	out, err := CustomInitAndApplyE(t, options)
	require.NoError(t, err)
	return out
}

//CustomInitAndApplyE runs terraform init (with custom azurerm plugin) and apply with the given options and return stdout/stderr from the apply command. Note that this
// method does NOT call destroy and assumes the caller is responsible for cleaning up any resources created by running
// apply.
func CustomInitAndApplyE(t testing.TestingT, options *Options) (string, error) {
	if _, err := CustomInitE(t, options); err != nil {
		return "", err
	}
